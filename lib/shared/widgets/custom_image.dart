...
import 'package:education_live_app/core/cach/cach_manger.dart';
  ...
  Widget build(BuildContext context) {
    ...
    switch (imageType) {
      ...
      case ImagesType.networkSvg:
        if (!_isValidUrl(_securePath)) return placeholder;
...
class _PlaceholderImage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final containerWidth = width ?? 100;
    final containerHeight = height ?? 100;
    ...
    if (isCircular) {
      return Container(
        ...
        decoration: BoxDecoration(
          ...
          color: context.colors.selektonizerLaoding,
        ),
      );
    }
    return Container(
      width: containerWidth == double.infinity
          ? double.infinity
          : containerWidth,
      height: containerHeight == double.infinity ? 100 : containerHeight,
      decoration: BoxDecoration(
        color: context.colors.selektonizerLaoding,
...
class ErrorImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (_isSvg(path)) {
      return SvgPicture.asset(path, height: h, width: w, fit: boxFit);
    }

    return Image.asset(path, height: h, width: w, fit: boxFit);
  }
}
